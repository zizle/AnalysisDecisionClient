...
class WarehouseVarietyManager(QWidget):
    ...
    def get_current_house_message(self):
        try:
            r = requests.get(url=SERVER_API + 'warehouse/' + str(self.house_code) + '/variety/')
            response = json.loads(r.content.decode('utf8'))
            ...
            self.table_show_content(response['varieties'])
...
class WarehouseAdmin(QWidget):
    ...
    def get_warehouses(self):
        try:
            r = requests.get(url=SERVER_API + 'warehouse/')
            response = json.loads(r.content.decode('utf8'))
            if r.status_code != 200:
                raise ValueError(response['message'])
        except Exception as e:
...
class HouseNumberAdmin(QWidget):
    ...
    def get_all_warehouse(self):
        # 获取所有仓库编号信息
        try:
            r = requests.get(url=SERVER_API + 'warehouse-number/', headers={'User-Agent': USER_AGENT})
            response = json.loads(r.content.decode('utf8'))
            if r.status_code !=200:
                raise ValueError(response['message'])
        except Exception as e:
    ...
    def add_new_house(self):
        def commit_new():
            try:
                r = requests.post(url=SERVER_API + 'warehouse-number/', headers={'Content-Type':'application/json;charset=utf8','User-Agent': USER_AGENT},
                                  data=json.dumps({'warehouse_short_name': popup.name_edit.text().strip()}))
                response = json.loads(r.content.decode('utf8'))
                if r.status_code != 201:
                    raise ValueError(response['message'])
            except Exception as e:
